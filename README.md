...
Если `topology` не задан, генератор пишет в лог, что использует `star`. Если указан неизвестный тип, он также откатывается к `star` и пишет предупреждение. Если один из endpoints отсутствует на текущей странице, такая связь пропускается на этой странице с предупреждением.
