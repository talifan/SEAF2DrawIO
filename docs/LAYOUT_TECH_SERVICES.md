## Назначение
`scripts/layout_tech_services.py` — постпроцессор, который дополняет результат `seaf2drawio.py`. Он собирает технические сервисы (compute, k8s, monitoring, kb и т.п.), группирует их по типам и раскладывает по сегментам ЦОДа. Внутри сегмента слева остаются сети, справа строятся сетки техсервисов, а рамка ЦОДа автоматически расширяется, чтобы вмещать все сегменты.

Важно: скрипт ориентирован именно на технические сервисы. Для страниц без таких объектов (например, офисный лист с сетями и пользовательскими устройствами) он выводит `technical services not found on the page; nothing to reflow.` и ничего не меняет.
...
- `scripts/layout_tech_services.py`
  - `--diagram all` — обработать все страницы, название которых содержит слова из `--diagram-filter` (по умолчанию `DC,ЦОД,Office,Офис,Головной`). Можно задать явный список: `--diagram "Sber Cloud DC,VK DC"`.
...
`SECURITY_ZONE_HINTS` привязывает объекты `seaf.company.ta.services.kb` по ключевым словам. Логика:
